...
import { findChatResponse } from "@/lib/chat-responses"
...
export default function MuseumChatbot() {
  ...
  const handleSendMessage = (message: string) => {
    ...
    setTimeout(() => {
      ...
      const matchedResponse = findChatResponse(message, language)

      if (matchedResponse) {
        // Handle special actions
        if (matchedResponse.response.includes("booking process")) {
          setShowTicketBooking(true)
          return
        }

        if (matchedResponse.response.includes("cancellation process")) {
          handleOpenCancelBookingFlow()
          return
        }

        // Create bot response with matched content
        const botResponse: MessageType = {
          id: (Date.now() + 1).toString(),
          role: "assistant",
          content: matchedResponse.response,
          type: matchedResponse.type || "quickReplies",
          options: matchedResponse.options || ["anythingElse", "returnToMenu"],
        }
      ...
      <AnimatePresence mode="wait">
        {!isChatOpen ? (
          ...
          >
            <Button
              onClick={toggleChat}
              className="h-14 w-14 rounded-full bg-black text-white shadow-lg hover:bg-gray-800"
            >
            ...
            <div className="absolute right-full top-1/2 -translate-y-1/2 mr-2">
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                transition={{ duration: 0.3 }}
                className="rounded-lg bg-gray-800 px-3 py-2 text-sm text-yellow-600 font-bold shadow-md whitespace-nowrap border border-yellow-600"
              >
